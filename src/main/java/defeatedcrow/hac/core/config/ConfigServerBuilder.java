...
public class ConfigServerBuilder {
	...
	ConfigServerBuilder(ForgeConfigSpec.Builder builder) {
		...
		builder.comment("========= Date Setting ========", "Setting for date in the game.")
		    .push("date_config");

		this.vYear = builder.comment("Set the number of days in the year.")
		    .defineInRange("Length of the Year", 240, 1, 365);

		this.vStartSpr = builder.comment("Set the dates for the beginning of Spring.")
		    .defineInRange("Start of Spring", 1, 1, 365);

		this.vStartSmr = builder.comment("Set the dates for the beginning of Summer.")
		    .defineInRange("Start of Summer", 61, 1, 365);

		this.vStartAut = builder.comment("Set the dates for the beginning of Autumn.")
		    .defineInRange("Start of Autumn", 121, 1, 365);

		this.vStartWtr = builder.comment("Set the dates for the beginning of Winter.")
		...
		this.enRealTime = builder.comment("Set the date format used in  real-date settings.")
		    .define("Enable Real Time", false);

		this.setFormat = builder.comment("Sets the date display format.")
	...
	public static void setDateFormat() {
		...
		} else if (s.contains("yy") || s.contains("MM") || s.contains("dd")) {
	...
	static {
		Pair<ConfigServerBuilder, ForgeConfigSpec> pair = new ForgeConfigSpec.Builder().configure(ConfigServerBuilder::new);
