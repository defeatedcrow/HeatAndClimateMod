...
public class EntityFireBarrier extends EntityMobBarrier {
	...
	public void onUpdate() {
		...
		if ((totalAge & 31) == 0 && !world.isRemote) {
			burnBlock();
			burnEntity();
		}
	}
	...
	public void burnBlock() {
		int sx = this.getPosition().getX() - 4;
		int sy = this.getPosition().getY() - 1;
		int sz = this.getPosition().getZ() - 4;
		int mx = this.getPosition().getX() + 4;
		int my = this.getPosition().getY() + 2;
		int mz = this.getPosition().getZ() + 4;
