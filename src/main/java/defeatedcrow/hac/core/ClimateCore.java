...
public class ClimateCore {
	...
	private static final RegistrySetBuilder BUILDER = new RegistrySetBuilder().add(Registries.DAMAGE_TYPE, DamageTypeInit::bootstrap);
	...
	public void gatherData(GatherDataEvent event) {
		...
		generator.addProvider(event.includeServer(), new DatapackBuiltinEntriesProvider(output, lookup, BUILDER, Set.of(ClimateCore.MOD_ID)));
		...
		generator.addProvider(event.includeServer(), new ItemTagProviderDC(output, lookup, blockTag, existingFileHelper));
	...
	private static java.util.concurrent.CompletableFuture<TagsProvider.TagLookup<Block>> safeBlockTags(java.util.concurrent.CompletableFuture<TagsProvider.TagLookup<Block>> inner) {
