...
@OnlyIn(Dist.CLIENT)
public class RenderSpoon extends EntityRenderer<ObjectEntityBaseDC> {

	protected CutleryModel model;

	public RenderSpoon(Context ctx) {
		super(ctx);
		this.model = new CutleryModel(ctx.bakeLayer(CutlerySpoonItem.SPOON.getLayerLocation()));
	}

	@Override
	public ResourceLocation getTextureLocation(ObjectEntityBaseDC entity) {
		return CutlerySpoonItem.SPOON.getTextureLocation();
	}
	...
	public void render(ObjectEntityBaseDC entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity != null) {
			...
			if (item instanceof IEntityItem && ((IEntityItem) item).getRenderData(item) != null) {
				...
				this.model.renderSpoon(poseStack, vertex, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
				poseStack.popPose();
			}
		}
