...
public class VeinTableRegister {
	...
	private static void addTable(VeinTable tableIn) {
		for (VeinTable table : INSTANCE.list) {
			String name = table.veinName;
			if (tableIn.veinName.equalsIgnoreCase(name)) {
				return;
			}
		}
		list.add(tableIn);
	...
	public static void initRegistry() {
		VeinTable white = new VeinTable("white", EnumVein.WHITE.toString(), 60, 40, 4, false, DCUtil.getBlockRegName(Blocks.BASALT))
		...
		VeinTable blue = new VeinTable("blue", EnumVein.BLUE.toString(), 60, 20, 4, false, DCUtil.getBlockRegName(Blocks.CALCITE))
				.setData(new OreSet(50, Blocks.CALCITE, CoreInit.ORE_FLUORITE.get(), 5)).setData(new OreSet(50, CoreInit.ORE_BLUE.get()));
		addTable(blue);

		VeinTable black = new VeinTable("black", EnumVein.BLACK.toString(), 60, 40, 4, false, DCUtil.getBlockRegName(Blocks.CLAY))
		...
		VeinTable red = new VeinTable("red", EnumVein.RED.toString(), 60, 20, 4, false, DCUtil.getBlockRegName(Blocks.RED_SANDSTONE))
				.setData(new OreSet(60, CoreInit.STONE_GYPSUM.get(), CoreInit.ORE_DESERTROSE.get(), 5)).setData(new OreSet(40, CoreInit.ORE_RED.get()));
		addTable(red);

		VeinTable green = new VeinTable("white", EnumVein.GREEN.toString(), 60, 20, 4, false, DCUtil.getBlockRegName(Blocks.DIORITE))
		...
		VeinTable white_blue = new VeinTable("white_blue", EnumVein.WHITE_BLUE.toString(), 30, -30, 8, true, DCUtil.getBlockRegName(Blocks.CALCITE))
		...
		VeinTable blue_black = new VeinTable("blue_black", EnumVein.BLUE_BLACK.toString(), 30, -30, 8, true, DCUtil.getBlockRegName(CoreInit.STONE_MUD.get()))
		...
		VeinTable black_red = new VeinTable("black_red", EnumVein.BLACK_RED.toString(), 30, -30, 8, true, DCUtil.getBlockRegName(CoreInit.STONE_GRANITE.get()))
		...
		VeinTable red_green = new VeinTable("red_green", EnumVein.RED_GREEN.toString(), 30, -30, 8, true, DCUtil.getBlockRegName(Blocks.TUFF))
		...
		VeinTable green_white = new VeinTable("green_white", EnumVein.GREEN_WHITE.toString(), 30, -30, 8, true, DCUtil.getBlockRegName(Blocks.GRANITE))
		...
		VeinTable nether = new VeinTable("nether", EnumVein.NETHER.toString(), 30, 60, 8, false, DCUtil.getBlockRegName(Blocks.NETHERRACK))
