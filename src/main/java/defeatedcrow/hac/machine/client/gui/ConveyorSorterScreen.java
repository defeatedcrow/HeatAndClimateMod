...
public class ConveyorSorterScreen extends AbstractContainerScreen<ConveyorSorterMenu> {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		// lock
		boolean lock = this.getMenu().getContainer().isLocked();
		if (lock) {
			pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}

		pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i + x1 + 1, j + y1 + 11, 189, this.menu.getFrontSortingType().priority * 8, 29, 8);
		pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i + x2 + 1, j + y2 + 11, 189, this.menu.getLeftSortingType().priority * 8, 29, 8);
		pose.blit(DCTexturePath.GUI_CONVEYOR_SORTER.getLocation(), i + x3 + 1, j + y3 + 11, 189, this.menu.getRightSortingType().priority * 8, 29, 8);
