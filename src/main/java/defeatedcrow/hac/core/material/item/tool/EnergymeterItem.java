		...
		if (res != null) {
			if (!level.isClientSide && player instanceof ServerPlayer sp) {
				if (level.getBlockEntity(res.getBlockPos()) != null) {
					level.getBlockEntity(res.getBlockPos()).getCapability(ForgeCapabilities.ENERGY).ifPresent(handler -> {
						int cap = handler.getMaxEnergyStored();
						int energy = handler.getEnergyStored();
						MutableComponent mes = Component.translatable("dcs.tip.energy").append(Component.literal(": ")).append(Component.literal(energy + "/" + cap + " FE"))
								.withStyle(ChatFormatting.AQUA);
						sp.sendSystemMessage(mes);
						player.swing(hand, true);
					});
				}
				BlockState state = level.getBlockState(res.getBlockPos());
				int rs = state.getSignal(level, res.getBlockPos(), res.getDirection().getOpposite());
				if (rs > 0) {
					MutableComponent mes = Component.translatable("dcs.tip.redstone").append(Component.literal(": ")).append(Component.literal(rs + "/15")).withStyle(ChatFormatting.AQUA);
					sp.sendSystemMessage(mes);
				}
				int inv = state.getAnalogOutputSignal(level, res.getBlockPos());
				if (inv > 0) {
					MutableComponent mes = Component.translatable("dcs.tip.comparator").append(Component.literal(": ")).append(Component.literal(inv + "/15")).withStyle(ChatFormatting.AQUA);
					sp.sendSystemMessage(mes);
				}
