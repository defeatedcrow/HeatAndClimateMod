...
public class DoubleInventoryScreen extends SimpleInventoryScreen {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(DCTexturePath.GUI_INV_DOUBLE.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		boolean lock = this.getMenu().getContainer().isLocked();
		if (lock) {
			pose.blit(DCTexturePath.GUI_INV_DOUBLE.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(DCTexturePath.GUI_INV_DOUBLE.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}
	}
}
