...
○v4.0.0pre
 ...
 add: Entity花瓶
 add: カフェカーテン
 change: ドーナツの当たり判定の高さを半分にした。
 change: カウンターチェアのアイテムアイコンを変更。
 change: 漬物アイテムをEntity化した。
 change: ツールチップの拡張表示キーをコンフィグで変更可能にした。
 fix: 彼岸花の球根のテクスチャを修正。
 fix: いくつかの花アイテムのTagの付け忘れを修正。
 fix: いくつかの料理のEntityが正常に表示されないのを修正。
 fix: 米粉麺とだんごのレシピかぶりを修正。
 fix: 無限水源蛇口の挙動を修正した。
 fix: 内部コードの細かな修正。
 ex: 開発環境をforge43.5.0に更新。
 
○v4-beta42 (2026.5.31)
 fix: 桜の苗からブナの葉が生成するのを修正。
 fix: マシンブロックを破壊したときのパーティクルを修正。
 fix: 液体燃料コンロのGUIのアイコンのずれを修正。
 
○v4-beta41 (2026.5.15)
 fix: ククイの苗の変異時にクラッシュするのを修正。
 fix: 鯛茶漬けのテクスチャを修正。

○v4-beta40 (2026.5.14)
 add: 樹皮
 add: 樹皮を剥いだ桜、梶、シナモンの原木
 change: アサガオは2ブロックの高さになった。
 change: いくつかの原木は斧で樹皮を剥がせるようにした。
 change: 苗木から育った原木にWILDフラグを付与し、斧で破壊したときに木こり効果を発揮するように。
 change: 木材系ブロックの可燃性を設定した。
 change: 葉ブロックからドロップする苗木アイテムの生成確率を減少させた。
 fix: 肥料アイテムを作物に使用できない。
 fix: 屋根がない座標は通気TIGHTにならないように。
 fix: キャッサバの苗が出現しない。
...
○v4-beta39 (2026.5.8)
 ...
 add: シナモンドーナツ
 add: ござ
 change: 砂利をクラフトして火打ち石を作成するレシピを追加した。
 change: 麦飯は大麦だけでも作れる。
 change: ニットのタイツのモデルをより細くした。
 fix: 鍋に入ったスープのテクスチャを修正。
 fix: カウンターチェアの高さを調整。
 fix: カウンターチェアのテクスチャを修正。

○v4-beta38 (2026.5.5)
 add: カウンターチェア
 add: レザーのチャップス
 add: リネンのズボンのカラーバリエーション3種
 add: ケーキバッター
 add: ドーナツ6種
 add: キッシュのバリエーション3種
 change: 木綿のシャツの襟を少し詰めた。
 change: 魚粉、腐葉土を骨粉のように作物に使用できるようにした。
 fix: 一部のブロック用Tagの登録忘れ。
 fix: ククイのカヌーのドロップアイテムを修正。
...
○v4-beta37 (2026.5.2)
 ...
 fix: ツールチップの表示内容を修正。
 fix: 紙パックで液体を汲む動作が安定しないのを修正。
 fix: ツールの攻撃力を修正。
 fix: ツル植物の座標オフセットをオフにした。
...
○v4-beta35 (2026.4.12)
 add: ヒガンバナ科、ユリ科、トウダイグサ科、アカネ科の作物
 add: ジャスミン
 add: ローストしたコーヒー豆、コーヒー粉
 add: いくつかの料理
 add: ククイの木材と建材
 add: 毒の樹液
 add: アウトリガ－・カヌー
 change: 野生作物の増殖速度はタイプによって異なる。
 change: 調味料アイテムにツールチップを追加。
 change: 葉ブロックのモデルを変更。
 change: 野生作物の生成時、バイオームの湿度も参照する。
 change: 石臼とクラッシャーのレシピカテゴリを分離した。
 change: コンベアによるバニラ精錬の必要温度をKILNに緩和した。
 change: 植物油のボイラー燃料性能を下方修正した。
 fix: プレイヤーが放流していない魚が消滅してしまう不具合を修正した。
 fix: ネザーとエンドの気温に季節補正値が適用されていない。
...
○v4-beta34 (2026.3.24)
 ...
 add: 重曹
 add: フレッシュチーズ
 change: 自然生成されたWILD作物が勝手に増えるハードモードコンフィグを追加。
 vhange: 自然生成された作物はバニラの花のようにランダムな座標を持つ。
 change: 赤青の魔法カードを水中で使用して電撃漁ができるようになった。
 change: ライオンゴロシ、ウルシの葉に接触時のダメージを設定。
 change: 色に対応するBiomeTagを少し修正。
 change: 内部コードのフォーマット。
 fix: 釣りで使用する魚のTagを一部修正。
 fix: 作物の成長速度の処理を修正。
 fix: ネギ類が最後まで成長しないのを修正。
 fix: 施肥された耕地に肥料を追加できないのを修正。
 fix: 樹皮の布の作成レシピが無い。

○v4-beta33 (2026.3.7)
 fix: 赤の矢のクラッシュを修正。
 fix: カラスマンタレットはロードチャンクの端にいるときに攻撃を停止するようにした。
...
○v4-beta31 (2026.2.17)
 ...
 change: ラゲッジやロッカーなどのエキス染色家具は設置後にも染色が可能に
 change: パイプの流体輸送仕様を変更し、低いところから充填されるようにした
 fix: マシンやチェストに付けたカスタムネームが設置時に消えてしまう
 fix: 負荷対策のため、赤の矢がアンロードされるとき、代わりに消滅するようにした
 fix: 吸気ファンに液体パイプ等がつながらない
 fix: 吸気ファンが圧縮空気を排出しない
 fix: ウルシの階段のテクスチャを修正
 fix: バッテリーブロックGUIの接続表示のミスを修正
