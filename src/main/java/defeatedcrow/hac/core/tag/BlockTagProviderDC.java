...
public class BlockTagProviderDC extends BlockTagsProvider {
	...
	protected void addTags() {
		...
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof IBlockDC && ((IBlockDC) block.get()).getToolType() == ToolType.PICKAXE).map(RegistryObject::get).forEach(pickaxe::add);

		TagsProvider.TagAppender<Block> axe = this.tag(BlockTags.MINEABLE_WITH_AXE);
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof IBlockDC && ((IBlockDC) block.get()).getToolType() == ToolType.AXE).map(RegistryObject::get).forEach(axe::add);

		TagsProvider.TagAppender<Block> toolTier1 = this.tag(BlockTags.NEEDS_STONE_TOOL);
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof IBlockDC && ((IBlockDC) block.get()).getToolTier() == 1).map(RegistryObject::get).forEach(toolTier1::add);

		TagsProvider.TagAppender<Block> toolTier2 = this.tag(BlockTags.NEEDS_IRON_TOOL);
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof IBlockDC && ((IBlockDC) block.get()).getToolTier() == 2).map(RegistryObject::get).forEach(toolTier2::add);

		TagsProvider.TagAppender<Block> toolTier3 = this.tag(BlockTags.NEEDS_DIAMOND_TOOL);
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof IBlockDC && ((IBlockDC) block.get()).getToolTier() == 3).map(RegistryObject::get).forEach(toolTier3::add);

		TagsProvider.TagAppender<Block> crops = this.tag(BlockTags.CROPS);
		CoreInit.BLOCKS.getEntries().stream().filter(block -> block.get() instanceof ClimateCropBaseBlock).map(RegistryObject::get).forEach(crops::add);
		...
		tag(TagDC.BlockTag.CROP_GREEN_MANURES).add(FoodInit.BLOCK_BR_RAPESEED.get(), FoodInit.BLOCK_BR_GREEN.get(),
		    ...
		    FoodInit.BLOCK_PE_GARBANZO.get(), FoodInit.BLOCK_PE_SOY.get(), FoodInit.BLOCK_RE_SORGHUM.get()).addTag(TagDC.BlockTag.WEED);
