...
public class HeatTreatmentCategory implements IRecipeCategory<IHeatTreatment> {
	...
	public HeatTreatmentCategory(IGuiHelper guiHelper) {
		...
		background = guiHelper.drawableBuilder(PluginTexDC.HEAT_TREATMENT.getLocation(), 18, 26, 140, 124).addPadding(0, 0, 9, 3).build();
	}
	...
	public void setRecipe(IRecipeLayoutBuilder builder, IHeatTreatment recipe, IFocusGroup focuses) {
		...
		for (DCHeatTier heat : heats) {
			builder.addSlot(RecipeIngredientRole.INPUT, 51 + heat.getID() * 6, 5).addIngredient(IngredientTypeDC.HEAT_TIER, heat).setCustomRenderer(IngredientTypeDC.HEAT_TIER, new HeatTierRenderer(6, 3));
		}

		List<DCHumidity> hums = recipe.getHeatingHum();
		for (DCHumidity hum : hums) {
			builder.addSlot(RecipeIngredientRole.INPUT, 51 + hum.getID() * 21, 11).addIngredient(IngredientTypeDC.HUMIDITY, hum).setCustomRenderer(IngredientTypeDC.HUMIDITY, new HumidityRenderer(21, 3));
		}

		List<DCAirflow> airs = recipe.getHeatingAir();
		for (DCAirflow air : airs) {
			builder.addSlot(RecipeIngredientRole.INPUT, 51 + air.getID() * 21, 17).addIngredient(IngredientTypeDC.AIRFLOW, air).setCustomRenderer(IngredientTypeDC.AIRFLOW, new AirflowRenderer(21, 3));
		}
		...
		if (!recipe.getCoolingTemp().isEmpty() && ConfigCommonBuilder.INSTANCE.enHeatTreatment.get()) {
			...
			for (DCHeatTier heat : heats2) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + heat.getID() * 6, 39).addIngredient(IngredientTypeDC.HEAT_TIER, heat).setCustomRenderer(IngredientTypeDC.HEAT_TIER, new HeatTierRenderer(6, 3));
			}

			List<DCHumidity> hums2 = recipe.getCoolingHum();
			for (DCHumidity hum : hums2) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + hum.getID() * 21, 45).addIngredient(IngredientTypeDC.HUMIDITY, hum).setCustomRenderer(IngredientTypeDC.HUMIDITY, new HumidityRenderer(21, 3));
			}

			List<DCAirflow> airs2 = recipe.getCoolingAir();
			for (DCAirflow air : airs2) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + air.getID() * 21, 51).addIngredient(IngredientTypeDC.AIRFLOW, air).setCustomRenderer(IngredientTypeDC.AIRFLOW, new AirflowRenderer(21, 3));
			}
		}
		...
		if (!recipe.getAnnealingTemp().isEmpty() && ConfigCommonBuilder.INSTANCE.enHeatTreatment.get()) {
			...
			for (DCHeatTier heat : heats3) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + heat.getID() * 6, 73).addIngredient(IngredientTypeDC.HEAT_TIER, heat).setCustomRenderer(IngredientTypeDC.HEAT_TIER, new HeatTierRenderer(6, 3));
			}

			List<DCHumidity> hums3 = recipe.getAnnealingHum();
			for (DCHumidity hum : hums3) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + hum.getID() * 21, 79).addIngredient(IngredientTypeDC.HUMIDITY, hum).setCustomRenderer(IngredientTypeDC.HUMIDITY, new HumidityRenderer(21, 3));
			}

			List<DCAirflow> airs3 = recipe.getAnnealingAir();
			for (DCAirflow air : airs3) {
				builder.addSlot(RecipeIngredientRole.INPUT, 51 + air.getID() * 21, 85).addIngredient(IngredientTypeDC.AIRFLOW, air).setCustomRenderer(IngredientTypeDC.AIRFLOW, new AirflowRenderer(21, 3));
			}
		}
	}

	@Override
	public void getTooltip(ITooltipBuilder tooltip, IHeatTreatment recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {}
	...
	public void draw(IHeatTreatment recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
		...
		if (!recipe.getCoolingTemp().isEmpty() && ConfigCommonBuilder.INSTANCE.enHeatTreatment.get()) {
			graphics.drawString(font, text1, 45, 26, 0xFF000000, false);
		}

		if (!recipe.getAnnealingTemp().isEmpty() && ConfigCommonBuilder.INSTANCE.enHeatTreatment.get()) {
			graphics.drawString(font, text1, 45, 60, 0xFF000000, false);
		}

		graphics.drawString(font, text1, 45, 93, 0xFF000000, false);
		MutableComponent text2 = Component.literal("Failure");
		graphics.drawString(font, text2, 80, 106, 0xFF000000, false);
