...
public class BlackRodScreen extends AbstractContainerScreen<BlackRodMenu> {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(DCTexturePath.GUI_BLACK_ROD.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		int select = this.getMenu().getSelectNum();
		pose.blit(DCTexturePath.GUI_BLACK_ROD.getLocation(), i + 26 + select * 26, j + 39, 176, 0, 20, 25);
