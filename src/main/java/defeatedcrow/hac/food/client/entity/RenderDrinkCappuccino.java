...
public class RenderDrinkCappuccino<T extends FoodEntityBase> extends RenderFoodBase<FoodEntityBase> {

	public RenderDrinkCappuccino(Context ctx) {
		super(ctx);
		this.model = new DrinkCupModel<>(ctx.bakeLayer(DrinkCappuccinoItem.LATTE.getLayerLocation()));
	}

	@Override
	public ResourceLocation getTextureLocation(FoodEntityBase entity) {
		return DrinkCappuccinoItem.LATTE.getTextureLocation();
	}
	...
	public void render(FoodEntityBase entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity != null) {
			...
			if (item instanceof IEntityItem && ((IEntityItem) item).getRenderData(item) != null) {
				...
				((DrinkCupModel<FoodEntityBase>) this.model).renderTop(poseStack, vertex, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
				poseStack.popPose();
			}
		}
	}
