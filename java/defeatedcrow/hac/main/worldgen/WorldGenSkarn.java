...
public class WorldGenSkarn implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
		...
		int f = world.rand.nextInt(3);
		int r = 12 + f;

		if ((genDim1 != 1 && genDim1 != -1)) {
			if (canGenerate(chunkX, chunkZ, world) && !BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.WET)
	...
	private boolean canGenerate(int chunkX, int chunkZ, World world) {
		...
		int i = CoreConfigDC.skarnGen;
		return world.rand.nextInt(1000) < i;
	}
