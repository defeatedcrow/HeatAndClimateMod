...
public class EntityLightBullet extends EntityBulletDC {
	...
	private void onEffect(BlockPos pos) {
		if (!this.world.isRemote) {
			...
			if (this.world.getBlockState(p).getMaterial().isReplaceable()) {
				this.world.setBlockState(p, MainInit.lightOrb.getDefaultState(), 3);
			} else if (this.world.getBlockState(pos.up()).getMaterial().isReplaceable()) {
				this.world.setBlockState(pos.up(), MainInit.lightOrb.getDefaultState(), 3);
			}
