...
@OnlyIn(Dist.CLIENT)
public class TileRendererCabinet implements BlockEntityRenderer<CabinetTile> {

	protected BlockCabinetModel model;

	public TileRendererCabinet(BlockEntityRendererProvider.Context ctx) {
		this.model = new BlockCabinetModel(ctx.bakeLayer(CabinetTile.NORMAL.getLayerLocation()));
	}

	@Override
	public void render(CabinetTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int i2) {
		if (tile != null && tile.getBlockState() != null) {
			...
			float f = tile.getOpenNess(partialTicks) * 85F;
