...
public class TreeDataCategory implements IRecipeCategory<LeavesCropBlockDC> {
	...
	@Override
	public void draw(LeavesCropBlockDC recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
		CropTier tier = recipe.getTier();
		CropType type = recipe.getFamily();
		...
		for (EnumSeason s : recipe.flowerSeasons) {
			...
			drawTexturedModalRect(graphics.pose().last().pose(), ad + 37, 29, 90 + ad, 170, 4, 8);
		}

		for (EnumSeason s : recipe.cropSeasons) {
			int ad = s.season + s.id * 4;
			drawTexturedModalRect(graphics.pose().last().pose(), ad + 88, 29, 90 + ad, 170, 4, 8);
		}
