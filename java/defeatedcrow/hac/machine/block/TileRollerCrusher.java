...
public class TileRollerCrusher extends TileTorqueProcessor implements ITorqueReceiver {
	...
	public boolean onProcess() {
		if (currentRecipe != null) {
			...
			worldObj.rand.nextInt(100);

			if (!DCUtil.isEmpty(out) && worldObj.rand.nextInt(100) < chance0) {
				this.insertResult(out);
			}

			if (!DCUtil.isEmpty(sec) && worldObj.rand.nextInt(100) < chance1) {
				this.insertResult(sec);
			}

			if (!DCUtil.isEmpty(tert) && worldObj.rand.nextInt(100) < chance2) {
