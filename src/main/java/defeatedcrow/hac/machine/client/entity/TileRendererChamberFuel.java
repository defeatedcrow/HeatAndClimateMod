...
@OnlyIn(Dist.CLIENT)
public class TileRendererChamberFuel implements BlockEntityRenderer<BrickChamberTile> {

	protected BlockChamberFuelModel model;
	private BlockRenderDispatcher renderer;

	public TileRendererChamberFuel(BlockEntityRendererProvider.Context ctx) {
		this.model = new BlockChamberFuelModel(ctx.bakeLayer(DATA.getLayerLocation()));
		renderer = ctx.getBlockRenderDispatcher();
	}

	@Override
	public void render(BrickChamberTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int i2) {
	...
	public static final EntityRenderData DATA = new EntityRenderData("tile/chamber_fuel", 1F, 0F);
