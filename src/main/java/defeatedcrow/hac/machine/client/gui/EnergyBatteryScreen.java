...
public class EnergyBatteryScreen extends AbstractContainerScreen<EnergyBatteryMenu> {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		// lock
		boolean lock = this.getMenu().getContainer().isLocked();
		if (lock) {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}

		if (this.menu.isPowerd()) {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 31, j + 34, 176, 82, 14, 9);
		} else {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 31, j + 42, 176, 91, 14, 9);
		}

		if (this.menu.isActive()) {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 60, j + 27, 176, 62, 10, 10);
		}

		if (this.menu.isRS()) {
			pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 60, j + 44, 176, 72, 10, 10);
		}

		int m = this.menu.getBatteryCount();
		if (m > 0) {
			for (int k = 0; k < m; k++) {
				pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 33, j + 100 - (k * 5), 176, 100, 10, 4);
			}
		}

		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 32, 189, this.menu.getFaceIO(0) * 8, 27, 8);
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 45, 189, this.menu.getFaceIO(1) * 8, 27, 8);
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 58, 189, this.menu.getFaceIO(2) * 8, 27, 8);
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 71, 189, this.menu.getFaceIO(3) * 8, 27, 8);
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 84, 189, this.menu.getFaceIO(4) * 8, 27, 8);
		pose.blit(DCTexturePath.GUI_BATTERY.getLocation(), i + 120, j + 97, 189, this.menu.getFaceIO(5) * 8, 27, 8);
