...
@OnlyIn(Dist.CLIENT)
public class BubbleParticleDC extends BaseAshSmokeParticle {

	private final double movY;

	protected BubbleParticleDC(ClientLevel level, double x, double y, double z, double sx, double sy, double sz, float size, SpriteSet sprite) {
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType type, ClientLevel level, double x, double y, double z, double sx, double sy, double sz) {
			return new BubbleParticleDC(level, x, y, z, sx, sy, sz, 1.0F, this.sprites);
		}
	}
