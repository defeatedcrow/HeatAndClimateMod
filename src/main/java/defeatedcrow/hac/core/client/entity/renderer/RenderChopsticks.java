...
@OnlyIn(Dist.CLIENT)
public class RenderChopsticks extends EntityRenderer<ObjectEntityBaseDC> {

	protected CutleryModel model;

	public RenderChopsticks(Context ctx) {
		super(ctx);
		this.model = new CutleryModel(ctx.bakeLayer(CutleryChopsticksItem.CHOPSTICKS.getLayerLocation()));
	}

	@Override
	public ResourceLocation getTextureLocation(ObjectEntityBaseDC entity) {
		return CutleryChopsticksItem.CHOPSTICKS.getTextureLocation();
	}
	...
	public void render(ObjectEntityBaseDC entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity != null) {
			...
			if (item instanceof IEntityItem && ((IEntityItem) item).getRenderData(item) != null) {
				...
				this.model.renderChopsticks(poseStack, vertex, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
				poseStack.popPose();
			}
		}
