...
@OnlyIn(Dist.CLIENT)
public class TileRendererPipeNickelsilver implements BlockEntityRenderer<FluidPipeNickelsilverTile> {

	protected CableModel model;

	public TileRendererPipeNickelsilver(BlockEntityRendererProvider.Context ctx) {
		this.model = new CableModel(ctx.bakeLayer(NICKELSILVER.getLayerLocation()));
	}

	@Override
	public void render(FluidPipeNickelsilverTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int i2) {
	...
	protected ResourceLocation getCableTexture(FaceIO io, Block block) {
		if (io == FaceIO.INPUT) {
			return NICKELSILVER_INPUT.getTextureLocation();
		} else if (io == FaceIO.OUTPUT) {
			return NICKELSILVER_OUTLET.getTextureLocation();
		}
		return NICKELSILVER.getTextureLocation();
	}

	public static final EntityRenderData NICKELSILVER = new EntityRenderData("tile/pipe_nickelsilver", 1F, 0F);
	public static final EntityRenderData NICKELSILVER_INPUT = new EntityRenderData("tile/pipe_nickelsilver_input", 1F, 0F);
	public static final EntityRenderData NICKELSILVER_OUTLET = new EntityRenderData("tile/pipe_nickelsilver_outlet", 1F, 0F);
