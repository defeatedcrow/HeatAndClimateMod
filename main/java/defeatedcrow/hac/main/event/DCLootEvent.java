...
public class DCLootEvent {
	...
	public void onEvent(LootTableLoadEvent event) {
		if (event.getTable() != null) {
			...
			// 村
			if (event.getName().equals(LootTableList.CHESTS_VILLAGE_BLACKSMITH) && WorldGenConfig.chestVillage) {
			...
			// ダンジョン
			else if (event.getName().equals(LootTableList.CHESTS_SIMPLE_DUNGEON) && WorldGenConfig.chestDungeon) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// ピラミッド 赤白
			else if (event.getName().equals(LootTableList.CHESTS_DESERT_PYRAMID) && WorldGenConfig.chestPyramid) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// 廃坑 白緑
			else if (event.getName().equals(LootTableList.CHESTS_ABANDONED_MINESHAFT) && WorldGenConfig.chestMine) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// 要塞 黒青
					...
					.equals(LootTableList.CHESTS_STRONGHOLD_CORRIDOR) && WorldGenConfig.chestStronghold) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
					...
					.equals(LootTableList.CHESTS_STRONGHOLD_LIBRARY) && WorldGenConfig.chestStronghold) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// ネザー 赤黒
			else if (event.getName().equals(LootTableList.CHESTS_NETHER_BRIDGE) && WorldGenConfig.chestNether) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// ジャングル寺院 青緑
			else if (event.getName().equals(LootTableList.CHESTS_JUNGLE_TEMPLE) && WorldGenConfig.chestJungle) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
					pool.addEntry(new LootEntryItem(MainInit.gems_green, 15, 3, new LootFunction[] {
			...
			// 洋館
			else if (event.getName().equals(LootTableList.CHESTS_WOODLAND_MANSION) && WorldGenConfig.chestWoodland) {
				LootTable loot = event.getTable();
				LootPool pool = loot.getPool("main");
				if (pool != null) {
			...
			// 釣り
				...
				if (pool != null) {
					...
					pool.addEntry(new LootEntryItem(meat, 15, 1, new LootFunction[] {
							new SetMetadata(new LootCondition[0], new RandomValueRange(9))
					}, new LootCondition[0],
