...
@OnlyIn(Dist.CLIENT)
public class TileRendererPortableCan implements BlockEntityRenderer<PortableCanTile> {

	protected PortableCanModel model;
	private BlockRenderDispatcher renderer;

	public TileRendererPortableCan(BlockEntityRendererProvider.Context ctx) {
		this.model = new PortableCanModel(ctx.bakeLayer(PortableCanTile.NORMAL.getLayerLocation()));
		renderer = ctx.getBlockRenderDispatcher();
	}

	@Override
	public void render(PortableCanTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int i2) {
