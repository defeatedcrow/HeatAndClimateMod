...
public class EntityProjCryM extends EntityMagicProjBase {

	public EntityProjCryM(World worldIn) {
		super(worldIn);
	}

	public EntityProjCryM(World worldIn, double x, double y, double z) {
		super(worldIn, x, y, z);
	}

	public EntityProjCryM(World worldIn, EntityLivingBase shooter) {
		super(worldIn, shooter);
	}

	@Override
	public ItemStack getDropStack() {
		return new ItemStack(MagicInit.daggerMagic, 1, 6);
	}
	...
	protected boolean onGroundHit() {
		...
		for (int i = 0; i < list.size(); ++i) {
			...
			if (entity != null && entity instanceof EntityLivingBase) {
				...
				if (liv.isPotionActive(MainInit.gravity)) {
					PotionEffect eff = liv.getActivePotionEffect(MainInit.gravity);
					int dur = eff.getDuration();
					int amp = eff.getAmplifier();
					liv.addPotionEffect(new PotionEffect(MainInit.gravity, dur + 600, amp));
				} else {
					liv.addPotionEffect(new PotionEffect(MainInit.gravity, 600, 1));
				}
			}
		}
	...
	protected void onGroundClient() {
		...
		shock.setRBGColorF(0.65F, 0.95F, 0.65F);
		...
		while (i < 3) {
			...
			Particle star = new ParticleFallingStar.Factory().createParticle(0, worldObj, x, y, z, fx, fy, fz,
					new int[0]);
			star.setRBGColorF(0.65F, 0.95F, 0.65F);
