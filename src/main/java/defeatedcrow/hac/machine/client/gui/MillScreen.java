...
public class MillScreen extends AbstractContainerScreen<MillMenu> {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(DCTexturePath.GUI_MILL.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		boolean lock = this.getMenu().getContainer().isLocked();
		if (lock) {
			pose.blit(DCTexturePath.GUI_MILL.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(DCTexturePath.GUI_MILL.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}

		int l = this.menu.getBurnProgress();
		if (l > 0)
			pose.blit(DCTexturePath.GUI_MILL.getLocation(), i + 67, j + 54, 189, l * 12, 12, 12);

		int m = this.menu.getBatteryCount();
		if (m > 0)
			pose.blit(DCTexturePath.GUI_MILL.getLocation(), i + 13, j + 78 - m * 2, 176, 42, 12, m * 2);

		if (this.menu.isRS()) {
			pose.blit(DCTexturePath.GUI_MILL.getLocation(), i + 14, j + 43, 176, 58, 10, 10);
		}
	}
