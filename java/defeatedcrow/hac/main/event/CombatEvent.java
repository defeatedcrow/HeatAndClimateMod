				...
				if (ownerLiv.isPotionActive(MobEffects.INVISIBILITY)) {
					if (living instanceof EntityLiving) {
						...
						if (living.getAttackingEntity() == null) {
							double d1 = living.getPosition().distanceSq(ownerLiv.getPosition().getX(), mob.getPosition().getY(), ownerLiv.getPosition().getZ());
					...
					for (int i = 0; i < EntityEquipmentSlot.values().length; i++) {
						...
						if (!DCUtil.isEmpty(item)) {
							...
							if (ownerLiv.world.spawnEntity(drop)) {
								...
								ownerLiv.world.playSound(null, ownerLiv.posX, ownerLiv.posY, ownerLiv.posZ, SoundEvents.BLOCK_ANVIL_BREAK, SoundCategory.BLOCKS, 1.5F, 1.0F /
		...
		if (target != null && source != null) {
			if (source.getTrueSource() instanceof EntityTameable) {
				...
				if (amu && living.getOwner() instanceof EntityPlayer) {
					...
					target.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer) living.getOwner()), event.getAmount());
