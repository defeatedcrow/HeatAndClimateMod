...
public class BreadRoundRenderer<T extends FoodEntityBase> extends EntityRenderer<T> {

	protected final EntityModel<T> model;

	public BreadRoundRenderer(Context ctx) {
		super(ctx);
	...
	public void render(T entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity != null) {
			...
			if (item instanceof IEntityItem && ((IEntityItem) item).getRenderData(item) != null) {
				...
				model.setupAnim(entity, yaw, partialTicks, packedLight, f1, f2);
