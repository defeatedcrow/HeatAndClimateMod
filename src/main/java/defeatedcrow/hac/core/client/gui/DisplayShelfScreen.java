...
public class DisplayShelfScreen extends AbstractContainerScreen<DisplayShelfMenu> {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		pose.blit(this.getMenu().getContainer().getContainerSize() == 10 ? DCTexturePath.GUI_DISPLAY_DOUBLE_SHELF.getLocation() : DCTexturePath.GUI_DISPLAY_SHELF.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		boolean lock = this.getMenu().getContainer().isLocked();
		if (lock) {
			pose.blit(this.getMenu().getContainer().getContainerSize() == 10 ? DCTexturePath.GUI_DISPLAY_DOUBLE_SHELF.getLocation() : DCTexturePath.GUI_DISPLAY_SHELF.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(this.getMenu().getContainer().getContainerSize() == 10 ? DCTexturePath.GUI_DISPLAY_DOUBLE_SHELF.getLocation() : DCTexturePath.GUI_DISPLAY_SHELF.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}
	}
