...
public class RodWhite extends MagicJewelBase {

	public RodWhite() {
		super("rod_white_red", MagicColor.WHITE_RED, Rarity.EPIC, TagDC.ItemTag.MAGIC_STUFF);
	}
	...
	public InteractionResultHolder<ItemStack> onBlockHit(Level level, Player player, InteractionHand hand, ItemStack charm, BlockHitResult res) {
		if (!DCUtil.isEmpty(charm) && level instanceof ServerLevel serverLevel) {
			...
			if (!level.isOutsideBuildHeight(p1) && target.isFaceSturdy(level, p1, res.getDirection(), SupportType.CENTER) && level.getBlockState(p1).getMaterial().isReplaceable()) {
				...
				MsgEffectToC.sendToClient(serverLevel, p1, 41);
			}
		}
