...
public class KichenOvenMenu extends AbstractContainerMenu {

	private final KichenOvenTile container;
	...
	public KichenStoveTile getContainer() {
		return container;
	}

	public static KichenOvenMenu getMenu(int i, Inventory playerInv, KichenOvenTile cont) {
		return new KichenOvenMenu(MachineInit.CHAMBER_MENU.get(), i, playerInv, cont, new SimpleContainerData(8));
	}

	public KichenOvenMenu(MenuType<?> type, int s, Inventory playerInv, KichenOvenTile cont, ContainerData d) {
		super(type, s);
		...
		if (cont.canOpen(playerInv.player)) {
			this.addSlot(new Slot(cont, 0, 37, 25));
			this.addSlot(new MachineResultSlot(cont, 1, 37, 51));

			this.addSlot(new Slot(cont, 2, 78, 67));
			this.addSlot(new MachineResultSlot(cont, 3, 78, 92));
			this.addSlot(new Slot(cont, 4, 96, 67));
			this.addSlot(new MachineResultSlot(cont, 5, 96, 92));
			this.addSlot(new Slot(cont, 6, 114, 67));
			this.addSlot(new MachineResultSlot(cont, 7, 114, 92));
	...
	public int getBurnProgress2() {
		int i = this.data.get(6);
		int j = this.data.get(7);
		if (j == 0 || i == 0)
			return 0;
		int ret = i * 5 / j;
		if (i > 0 && ret == 0)
