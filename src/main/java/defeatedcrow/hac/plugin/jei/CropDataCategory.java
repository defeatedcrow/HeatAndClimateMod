...
public class CropDataCategory implements IRecipeCategory<ClimateCropBaseBlock> {
	...
	public CropDataCategory(IGuiHelper guiHelper) {
		...
		background = guiHelper.drawableBuilder(PluginTexDC.CROP.getLocation(), 21, 19, 134, 128).addPadding(0, 0, 10, 8).build();
	}
	...
	@Override
	public void getTooltip(ITooltipBuilder tooltip, ClimateCropBaseBlock recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
		CropTier tier = recipe.getTier();
