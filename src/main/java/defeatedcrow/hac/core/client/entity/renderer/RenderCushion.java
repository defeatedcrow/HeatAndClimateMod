...
@OnlyIn(Dist.CLIENT)
public class RenderCushion extends EntityRenderer<CushionEntity> {

	protected CushionSquareModel<CushionEntity> model;

	public RenderCushion(Context ctx) {
		super(ctx);
		this.model = new CushionSquareModel<>(ctx.bakeLayer(CushionItem.WHITE.getLayerLocation()));
	}

	@Override
	public ResourceLocation getTextureLocation(CushionEntity entity) {
		return CushionItem.WHITE.getTextureLocation();
	}

	@Override
	public void render(CushionEntity entity, float yaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity != null) {
