...
public class ConfigClientBuilder {
	...
	public final ForgeConfigSpec.IntValue key_Desc;

	public static int HUD_type = 0;

	ConfigClientBuilder(ForgeConfigSpec.Builder builder) {

		builder.comment("========= Render Setting ========", "Setting for rendering")
		    .push("render_config");

		this.showAltTip = builder.comment("Gives some additional tooltips to the item.")
		    .define("Show Additional Tooltips", true);

		this.showTasteTip = builder.comment("Display taste level in tooltips for non-HaC items.")
		    .define("Show Food Taste Tooltips", true);

		this.showDamEffect = builder.comment("Set the screen effect when climate damage occurs.")
		    .define("Show Damage Effect", true);

		this.showHUD = builder.comment("Show climate info on HUD.")
		    .define("Show Climate HUD", true);

		this.disablePotionEffect = builder.comment("Disable potion effects particles in first person view.")
		    .define("Disable potion particles in FPV", true);

		builder.pop();

		builder.comment("======== HUD Setting ========", "Setting for the HUD display.")
		    .push("hud_config");
		...
		this.offset_y = builder.comment(" Adjust the display position of the HUD.")
		    .defineInRange("HUD Y Offset", 160, -100, 240);

		builder.pop();

		builder.comment("======== Keybind Setting ========", "Setting for the keybind.")
		    .push("key_config");

		this.key_HUD = builder.comment(" Set the key to switch HUD design. default:R-shift")
		    .defineInRange("HUD Switch Key", 344, 32, 348);

		this.key_Charm = builder.comment(" Sets the using charm effect key. default:X")
		    .defineInRange("Charm Using Key", 88, 32, 348);

		this.key_Desc = builder.comment(" Sets the using item tooltip expantion. default:L-shift")
		    .defineInRange("Item Tooltip Key", 340, 32, 348);

		builder.pop();
	}

	static {
		Pair<ConfigClientBuilder, ForgeConfigSpec> pair = new ForgeConfigSpec.Builder().configure(ConfigClientBuilder::new);
