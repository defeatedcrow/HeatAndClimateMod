...
@OnlyIn(Dist.CLIENT)
public class DisplayCaseScreen_Owner extends AbstractContainerScreen<DisplayCaseMenu_Owner> {

	public DisplayCaseScreen_Owner(DisplayCaseMenu_Owner menu, Inventory playerInv, Component comp) {
	...
	protected void renderBg(GuiGraphics pose, float f, int mx, int my) {
		...
		RenderSystem.setShaderTexture(0, DCTexturePath.GUI_DISPLAY_CASE_OWNER.getLocation());

		int i = (this.width - this.imageWidth) / 2;
		int j = (this.height - this.imageHeight) / 2;
		pose.blit(DCTexturePath.GUI_DISPLAY_CASE_OWNER.getLocation(), i, j, 0, 0, this.imageWidth, this.imageHeight);

		boolean lock = this.getMenu()
		    .getContainer()
		    .isLocked();
		if (lock) {
			pose.blit(DCTexturePath.GUI_DISPLAY_CASE_OWNER.getLocation(), i + 156, j + 3, 176, 21, 12, 21);
		} else {
			pose.blit(DCTexturePath.GUI_DISPLAY_CASE_OWNER.getLocation(), i + 156, j + 3, 176, 0, 12, 21);
		}
	}
